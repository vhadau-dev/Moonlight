...
moon({
  ...
  async execute(sock, jid, sender, args, m, { reply }) {
    try {
      ...
      if (!mentionedJid) {
        return reply("❌ Tag someone to poke! Example: .poke @user");
      }
      ...
      await sock.sendMessage(jid, {
        video: buffer,
        caption: `👉 *@${sender.split("@")[0]}* poked *@${mentionedJid.split("@")[0]}*!`,
        gifPlayback: true,
        mimetype: 'video/mp4',
      ...
      console.error("poke command error:", err);
      reply("❌ Failed to send poke. Please try again later.");
    }
  }
});
