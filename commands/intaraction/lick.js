...
moon({
  ...
  async execute(sock, jid, sender, args, m, { reply }) {
    try {
      ...
      if (!mentionedJid) {
        return reply("❌ Tag someone to lick! Example: .lick @user");
      }

      const res = await axios.get("https://nekos.best/api/v2/lick");
      ...
      await sock.sendMessage(jid, {
        video: buffer,
        caption: `👅 *@${sender.split("@")[0]}* licked *@${mentionedJid.split("@")[0]}*!`,
        gifPlayback: true,
        mimetype: 'video/mp4',
      ...
      reply("❌ Failed to send lick. Please try again later.");
    }
  }
});
