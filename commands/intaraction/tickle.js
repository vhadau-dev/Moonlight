...
moon({
  ...
  async execute(sock, jid, sender, args, m, { reply }) {
    try {
      ...
      if (!mentionedJid) {
        return reply("❌ Tag someone to tickle! Example: .tickle @user");
      }
      ...
      await sock.sendMessage(jid, {
        video: buffer,
        caption: `🤏 *@${sender.split("@")[0]}* tickled *@${mentionedJid.split("@")[0]}*!`,
        gifPlayback: true,
        mimetype: 'video/mp4',
      ...
      console.error("tickle command error:", err);
      reply("❌ Failed to send tickle. Please try again later.");
    }
  }
});
